...
";
        ...
        [SKFunctionContextParameter(Name = Parameters.SearchPattern,
            Description = "The search string to match against the names of files in the repository")]
        public async Task SummarizeRepositoryAsync(string source, SKContext context)
        {
            if (!context.Variables.Get(Parameters.RepositoryBranch, out string repositoryBranch) ||
                string.IsNullOrEmpty(repositoryBranch))
            {
                repositoryBranch = "main";
            }

            if (!context.Variables.Get(Parameters.SearchPattern, out string searchPattern) ||
            ...
            try
            {
                ...
                await this.SummarizeCodeDirectoryAsync(directoryPath, searchPattern, repositoryUri, repositoryBranch,
                    context);
        ...
        /// </summary>
        private async Task SummarizeCodeFileAsync(string filePath, string repositoryUri, string repositoryBranch,
            string fileUri)
        {
        ...
        /// </summary>
        private async Task SummarizeCodeDirectoryAsync(string directoryPath, string searchPattern, string repositoryUri,
            string repositoryBranch, SKContext context)
        {
        ...
        /// </summary>
        private string BuildFileUri(string directoryPath, string filePath, string repositoryUri,
            string repositoryBranch)
        {
            var repositoryBranchName =
                $"{repositoryUri.Trim('/').Substring(repositoryUri.LastIndexOf('/'))}-{repositoryBranch}";
