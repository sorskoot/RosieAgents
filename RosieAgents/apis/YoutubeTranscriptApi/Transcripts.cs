...
namespace YoutubeTranscriptApi
{
    ...
    internal sealed class TranscriptListFetcher
    {
        ...
        {
            var splittedHtml = html.Split("\"captions\":");
            ...
            var captionsJson = JsonSerializer.Deserialize<JsonElement>(
                splittedHtml[1].Split(",\"videoDetails")[0].Replace("\n", "")
                ).GetProperty("playerCaptionsTracklistRenderer");

            if (!captionsJson.TryGetProperty("captionTracks", out _))
            {
        ...
        internal void CreateConsentCookie(string html, string videoId)
        {
            ...
            httpClientHandler.CookieContainer.Add(new Cookie("CONSENT", $"YES+{match.Groups[1].Value}", null, ".youtube.com"));
        }
        ...
        {
            string debugUrl = string.Format(Settings.WATCH_URL, videoId);
            var rawResult = httpClient.GetStringAsync(debugUrl).Result;
